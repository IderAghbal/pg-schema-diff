...
func buildDumpCmd() *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		compiledRegexes := make([]*regexp.Regexp, 0, len(excludeNameRegexes))
		for _, raw := range excludeNameRegexes {
			re, err := regexp.Compile(raw)
			if err != nil {
				return fmt.Errorf("compiling --exclude-name-regex %q: %w", raw, err)
			}
